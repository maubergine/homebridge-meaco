...
# actions/setup-node are JavaScript actions whose main and post steps run via
...
# their post steps run cleanly. The workflow's own setup-node step still installs
# the requested matrix version (22.x / 24.x) on top; this Node is only the
# bootstrap the JS actions need. scripts/ci-local.sh builds it automatically.
